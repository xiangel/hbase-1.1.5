...
public class ThriftServer {
  ...
  public static void main(String[] args) throws Exception {
    ...
    //if server is tthread pool server, can config the max and min worker thread number
    int minWorkerThreads = conf.getInt("hbase.thrift.minWorkerThreads", 200);
    int maxWorkerThreads = conf.getInt("hbase.thrift.maxWorkerThreads", 1000);
    ...
    if (nonblocking) {
      ...
      server = getTThreadPoolServer(protocolFactory, processor, transportFactory, inetSocketAddress, minWorkerThreads, maxWorkerThreads);
    }
